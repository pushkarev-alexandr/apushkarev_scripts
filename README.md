...
- **customSimpleBackdrop.py**: Custom backdrop creation, simplified version. Sets z_order correctly on creation, assigns a random backdrop color, makes the backdrop slightly wider relative to selected nodes than standard, sets font size relative to backdrop size
...
- **disconnectViewersOnLoad.py**: When opening the script, all inputs of all viewers are disconnected so that the tree is not processed and the script does not slow down
